...
echo "$JSON_DATA" > ../client/public/versionInfo.json

# Update the docker-compose.yml tag
echo "** Updating docker-compose.yml tag... to $CURRENT_BRANCH"
sed -i "s/image: iisdevs\/sotu-data-dashboard:main/image: iisdevs\/lac_sotu_dashboard_react:$CURRENT_BRANCH/g" ./docker-compose.yml
...
curl -sSL -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github.v3+json" -X POST https://api.github.com/repos/memphis-iis/lac_sotu_dashboard_react/releases -d "{\"tag_name\": \"$RELEASE_NAME\", \"name\": \"$RELEASE_NAME\", \"body\": \"$RELEASE_BODY\"}"
...
sudo docker login
sudo docker-compose build lac_sotu_dashboard_react
sudo docker-compose push lac_sotu_dashboard_react
